...
// Each query runs independently so one bad query never kills the whole dashboard.
router.get("/analytics/dashboard", requireAdmin, async (_req, res) => {
  async function q<T>(label: string, sql: string, params?: unknown[]): Promise<T[]> {
    try {
      const { rows } = await pool.query(sql, params);
      return rows as T[];
    } catch (err) {
      console.error(`Analytics query failed [${label}]:`, err);
      return [];
    }
  }

  // ── Daily active users — last 14 days ─────────────────────────────
  const dauRows = await q("dau", `
    SELECT
  ...
  const sectionRows = await q("sections", `
    SELECT
  ...
  const loginRows = await q("logins", `
    SELECT
      DATE(created_at AT TIME ZONE 'UTC') AS day,
      COUNT(*) AS logins,
      COUNT(DISTINCT member_email) AS unique_users
  ...
  const sessionRows = await q("sessions", `
    SELECT
  ...
  // ── Tutorial funnel ───────────────────────────────────────────────
  const tutorialStepRows = await q("tutorialSteps", `
    SELECT
    ...
    WHERE event_type = 'tutorial_step'
      AND metadata->>'step' ~ '^[0-9]+$'
    GROUP BY step
    ORDER BY step
  `);

  const tutorialOutcomeRows = await q("tutorialOutcomes", `
  ...
  const tutorialSkipRows = await q("tutorialSkips", `
    SELECT
  ...
  const wellCupMemberRows = await q("wellCupByMember", `
    SELECT
      al.member_email,
      m.name,
  ...
  const wellCupRecentRows = await q("wellCupRecent", `
    SELECT
      al.member_email,
      m.name,
      al.activity_type,
  ...
  // ── Overall summary ───────────────────────────────────────────────
  const summaryRows = await q("summary", `
    SELECT
  ...
  const retentionRows = await q("retention", `
    WITH first_open AS (
  ...
  const memberStatsRows = await q("memberStats", `
    SELECT
  ...
  const memberSectionRows = await q("memberSections", `
    SELECT
      member_email,
      metadata->>'section' AS section,
      COUNT(*) AS visits
  ...
  const forumCategoryRows = await q("forumByCategory", `
    SELECT
  ...
  // ── RSVP log — only RSVPs whose event still exists ────────────────
  const rsvpLogRows = await q("rsvpLog", `
    SELECT
      er.member_email,
      m.name,
      er.event_id,
      ...
      er.created_at
    FROM event_rsvps er
    LEFT JOIN members m ON m.email = er.member_email
  ...
  const rsvpEventRows = await q("rsvpEvents", `
    SELECT
      member_email,
  ...
  const streakRows = await q("streaks", `
    SELECT
      ls.member_email,
      m.name,
      ls.current_streak,
    ...
    LEFT JOIN members m ON m.email = ls.member_email
    ORDER BY ls.current_streak DESC, ls.longest_streak DESC
  `);

  // ── WELL Cup streak bonus totals ───────────────────────────────────
  const streakBonusRows = await q("streakBonuses", `
    SELECT
      al.member_email,
      m.name,
