...
export async function generateAIContent(targetDate?: string): Promise<void> {
  ...
  // Same 7-day lookback for inspirations and well activities so the AI
  // avoids the full recent window, not just the previous day.
  const { rows: recentInspirationRows } = await pool.query(
    `SELECT daily_inspiration FROM content_schedule
     WHERE date < $1 AND date >= $2 AND daily_inspiration IS NOT NULL
     ORDER BY date DESC`,
    [date, addDays(date, -7)]
  );
  const recentInspirationTitles = recentInspirationRows
    .map((r) => (r.daily_inspiration as { title?: string })?.title)
    .filter((t): t is string => !!t);

  const { rows: recentActivityRows } = await pool.query(
    `SELECT well_activity FROM content_schedule
     WHERE date < $1 AND date >= $2 AND well_activity IS NOT NULL
     ORDER BY date DESC`,
    [date, addDays(date, -7)]
  );
  const recentActivityTitles = recentActivityRows
    .map((r) => (r.well_activity as { title?: string })?.title)
    .filter((t): t is string => !!t);
  ...
  if (!row?.daily_inspiration) {
    try {
      const inspiration = await generateDailyInspiration(weeklyThemeTitle, recentInspirationTitles);
      await pool.query(
  ...
  if (!row?.well_activity) {
    try {
      const activity = await generateWellActivity(weeklyThemeTitle, recentActivityTitles);
      await pool.query(
...
// Generates a full week of content ahead of time (today through +7 days)
// instead of only generating each day's content the same morning it's due
// to post. Gives admins several days of lead time to preview and edit
// content in the admin panel before it ever goes out. Each date's
// generation only fills in fields that are still missing (see the COALESCE
// writes in generateAIContent above), so it's safe to re-run daily without
// clobbering anything an admin already edited or generated by hand.
export async function generateAIContentForUpcomingWeek(): Promise<void> {
  ...
  for (let i = 0; i <= 7; i++) {
    ...
    try {
      ...
      console.error(`[SCHEDULER] AI content generation failed for ${date}:`, err);
    }
  }
}
...
export function startScheduler(): void {
  // AI content generation (motivation boost, recipe, nutrition tip): every day
  // at 5:30am, tops up the next 7 days of content (not just today) so admins
  // have a week of lead time to preview/edit in the admin panel before
  // anything actually posts. Safe to re-run daily — already-filled fields
